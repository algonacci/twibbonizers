...
st.info("ℹ️ **Flexible Options:** You can use twibbon only, watermark only, or both together!")
...
st.sidebar.header("📌 Upload Templates (Optional)")
twibbon = file_uploader_with_preview("Upload Twibbon (PNG, Optional)", ["png"], "twibbon")
...
if st.sidebar.button("🚀 Process Images"):
    if not uploaded_images:
    ...
    elif not twibbon and not watermark:
        st.error("⚠️ Please upload at least a twibbon or a watermark!")
    else:
        # Convert uploaded files to PIL images - twibbon is now optional
        twibbon_img = Image.open(twibbon) if twibbon else None
        ...
        with st.expander("📊 Processing Summary"):
            ...
            st.write(f"🎭 **Twibbon Applied:** {'Yes' if twibbon else 'No'}")
