...
def process_images(images, twibbon, watermark, output_size, watermark_opacity, watermark_position, watermark_size):
    ...
    # Convert twibbon to RGBA only if it exists
    if twibbon:
        twibbon = twibbon.convert("RGBA")

    # Convert watermark to RGBA only if it exists
    if watermark:
    ...
    for image_file in images:
        ...
        # Apply twibbon only if it exists
        if twibbon:
            # Resize twibbon to match image size
            if output_size[1]:
                twibbon_resized = twibbon.resize(output_size, Image.LANCZOS)
            else:
                twibbon_resized = twibbon.resize((output_size[0], img_resized.height), Image.LANCZOS)

            # Apply twibbon
            combined = Image.alpha_composite(combined, twibbon_resized)
